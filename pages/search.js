    ...
    <div>
      {posts.total_rows ? (
        <div className="bg-white md:rounded-xl p-4 divide-y-2">
          <h1 className="font-title text-4xl pb-4">
        ...
        <div className="ml-4 p-4 text-lg">
          <h1 className="font-title text-4xl py-2">
            No se han encontrado posts para la consulta {`"${q}"`}
          </h1>
          <p className="font-body">
            Para más información sobre como realizar búsquedas avanzadas, ver el{" "}
            <a
...
export async function getServerSideProps(context) {
  ...
  const endpoint = `${process.env.NGINX_URL || "http://localhost:5984"}/${
    process.env.DBNAME
  }/_design/search/_search/post`;
